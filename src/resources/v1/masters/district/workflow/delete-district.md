# Delete District Workflow

**Title:** Soft/Force Delete District
...
flowchart TD
    ...
    E --> F(deleteDistrictService.execute)
    F --> G[Start Mongoose Session & Transaction]
    G --> H[Check if District has dependent records in other collections]
...
- If district ID is invalid -> `invalid_id`
- If district does not exist -> `district_not_found`
- Relational check for other modules using this district.
...
- Data: Deleted district document details
