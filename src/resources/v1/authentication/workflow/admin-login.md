...
**Title:** Admin User Login with Credentials

## **Workflow Diagram**

```mermaid
flowchart TD
    A[Client Request] --> B(Express Router /admin-login)
    B --> C{validationMiddleware}
    C -- Invalid credentials format --> D[400 Bad Request]
    C -- Valid format --> E(authenticationController.AdminLogin)
    E --> F(LoginAdminService.execute)
...
| --- | --- |
| LoginAdminService | Coordinates the transaction lifecycle and login logic execution for admin users. |
