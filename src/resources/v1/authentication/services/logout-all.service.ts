...
class logoutAllService {
  ...
  public async execute(
    ...
    try {
      ...
      await this.getSession(payload.jti, session);

      await this.logoutSession(payload.id, session, dbTransactions);
  ...
  private async logoutSession(
    ...
    try {
      const authSession = await this.authSessionRepository.updateMany(
        { userId: userId },
